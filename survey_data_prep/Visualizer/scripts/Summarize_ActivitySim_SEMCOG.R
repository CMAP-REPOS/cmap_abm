...
modeCodes <- data.frame(code = c(1, 2, 3, 4, 5, 6, 6, 6, 7, 7, 7, 8, 8, 8, 9),
...
per$HDISTRICT <- xwalk$COUNTY_NAME[match(per$home_taz, xwalk$TAZ)]
per$WDISTRICT <- xwalk$COUNTY_NAME[match(per$workplace_taz, xwalk$TAZ)]
...
hh$COUNTYFP10 <- xwalk$COUNTY[match(hh$TAZ, xwalk$TAZ)]
...
students <- per[per$school_taz > 0 & per$is_student == "True",]
...
workTripLengths <- ddply(workers[,c("HDISTRICT", "distance_to_work", "finalweight")], c("HDISTRICT"), summarise, work = weighted.mean(distance_to_work, finalweight))
...
univTripLengths <- ddply(students[students$PERTYPE==3,c("HDISTRICT", "distance_to_school", "finalweight")], c("HDISTRICT"), summarise, univ = weighted.mean(distance_to_school, finalweight))
...
schlTripLengths <- ddply(students[students$PERTYPE>=6,c("HDISTRICT", "distance_to_school", "finalweight")], c("HDISTRICT"), summarise, schl = weighted.mean(distance_to_school, finalweight))
...
mandTripLengths <- merge(workTripLengths, univTripLengths, by='HDISTRICT', all=TRUE)
mandTripLengths <- merge(mandTripLengths, schlTripLengths, by='HDISTRICT', all=TRUE)
...
tod1 <- wtd.hist(tours$tourdur[tours$TOURPURP==1], breaks = seq(0,18, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==1])
tod2 <- wtd.hist(tours$tourdur[tours$TOURPURP==2], breaks = seq(0,18, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==2])
tod3 <- wtd.hist(tours$tourdur[tours$TOURPURP==3], breaks = seq(0,18, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==3])
tod4 <- wtd.hist(tours$tourdur[tours$TOURPURP==4], breaks = seq(0,18, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==4])
todi56 <- wtd.hist(tours$tourdur[tours$TOURPURP>=5 & tours$TOURPURP<=6], breaks = seq(0,18, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=5 & tours$TOURPURP<=6])
todi789 <- wtd.hist(tours$tourdur[tours$TOURPURP>=7 & tours$TOURPURP<=9], breaks = seq(0,18, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=7 & tours$TOURPURP<=9])
todj56 <- wtd.hist(unique_joint_tours$tourdur[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6], breaks = seq(0,18, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$finalweight[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6])
todj789 <- wtd.hist(unique_joint_tours$tourdur[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9], breaks = seq(0,18, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$finalweight[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9])
tod15 <- wtd.hist(tours$tourdur[tours$TOURPURP==10], breaks = seq(0,18, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==10])
...
avgTripLengths <- c(weighted.mean(tours$SKIMDIST[tours$TOURPURP==4], tours$finalweight[tours$TOURPURP==4], na.rm = TRUE),
...
# ---------------------------------
#Work
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==9])
...
#University
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==9])
...
#School
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==9])
...
#iMain
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==9])
...
#iDisc
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==9])
...
#jMain
tripmode1 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==1])
tripmode2 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==2])
tripmode3 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==3])
tripmode4 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==4])
tripmode5 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==5])
tripmode6 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==6])
tripmode7 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==7])
tripmode8 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==8])
tripmode9 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==9])
...
#jDisc
tripmode1 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==1])
tripmode2 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==2])
tripmode3 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==3])
tripmode4 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==4])
tripmode5 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==5])
tripmode6 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==6])
tripmode7 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==7])
tripmode8 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==8])
tripmode9 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==9])
...
#At work
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==9])
...
#iTotal
itripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==1])
itripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==2])
itripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==3])
itripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==4])
itripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==5])
itripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==6])
itripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==7])
itripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==8])
itripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==9])

#jTotal
jtripmode1 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==1], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==1])
jtripmode2 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==2], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==2])
jtripmode3 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==3], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==3])
jtripmode4 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==4], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==4])
jtripmode5 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==5], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==5])
jtripmode6 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==6], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==6])
jtripmode7 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==7], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==7])
jtripmode8 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==8], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==8])
jtripmode9 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==9], breaks = seq(1,10, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==9])
...
total_vmt <- sum((trips$finalweight[trips$TRIPMODE<=3]*trips$od_dist[trips$TRIPMODE<=3])/trips$num_travel[trips$TRIPMODE<=3]) + 
  sum((jtrips$finalweight[jtrips$TRIPMODE<=3]*jtrips$od_dist[jtrips$TRIPMODE<=3])/jtrips$num_travel[jtrips$TRIPMODE<=3])
