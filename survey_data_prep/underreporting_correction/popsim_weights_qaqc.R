...
h_initial  = fread(file = file.path(settings$data_dir,settings$cmap_folder, 'household.csv'))
...
weights = merge(popsim_weights, h_survey_data, on = 'sampno')
weights = merge(weights, h_initial[, .(sampno, wthhfin)], on = 'sampno')
...
ggplot(data = weights, aes(x = wthhfin/initial_expansion_factor)) + geom_histogram() + 
  xlab('Ratio of Previous Weight:Initial expansion factor') + xlim(0, 3.5) + ylim(0, 4500)
