...
# # Smartphone type
fit_dt[, owns_phone := 1 * (smrtphn == 1 | app_data == 1)]  # why some don't own phone but participated by smartphone
...
fit_dt_nirpc = fit_dt_adult[study == 'nirpc']
model_nirpc = multinom(
  update(model_rhs, day_trip_class ~ .),
  data = fit_dt_nirpc,
  weights = fit_dt_nirpc[,estimation_weight],
  model = TRUE, Hess = TRUE)
...
options(scipen = 999)
tidy.opts = options(scipen = 99)
print(broom::tidy(model_nirpc, exponentiate = FALSE), n = 400)

t(summary(model_nirpc)$coefficients / summary(model_nirpc)$standard.errors)

# fitted(model)

# rho-square
DescTools::PseudoR2(model_nirpc) # Ohio: 0.164; # Baton Rouge 0.247

saveRDS(broom::tidy(model_nirpc, exponentiate = FALSE), file.path(wt_dir,"model_summaries","day_pattern_model_coef_new_nirpc.rds"))
...
# checks out
all.equal(
  colSums(predict(model_nirpc, newdata = fit_dt_nirpc, type = "probs") * fit_dt_nirpc[, estimation_weight]),
  fit_dt_nirpc[, .(weighted_sum = sum(estimation_weight)), .(day_trip_class)][order(day_trip_class), weighted_sum]
)

fit_dt_nirpc[, model_predict_none := predict(model_nirpc, newdata = fit_dt_nirpc, type = "probs")[, 'none']]
fit_dt_nirpc[, model_predict_mand := predict(model_nirpc, newdata = fit_dt_nirpc, type = "probs")[, 'mandatory']]
fit_dt_nirpc[, model_predict_non_mand := predict(model_nirpc, newdata = fit_dt_nirpc, type = "probs")[, 'non-mandatory']]

fit_dt_nirpc[, sum(model_predict_none * estimation_weight), by = .(online_data)]

# Recalculate predicteds without bias parameters removed
# rMove people tend to record more trips

model_coefs_nirpc = coef(model_nirpc)

model_coefs_nirpc[, "online_data"] = 0
# model_coefs[, "was_proxied_online_data"] = 0
# model_coefs[, "online_data:age_18_35"] = 0

V_none = 0
V_mandatory = model.matrix(model_rhs, fit_dt_nirpc) %*% t(t(model_coefs_nirpc['mandatory',]))
V_non_mand = model.matrix(model_rhs, fit_dt_nirpc) %*% t(t(model_coefs_nirpc['non-mandatory',]))
...
# Expect fewer "none" trips in model with bias parameters removed.
colSums(P)
colSums(predict(model_nirpc, newdata = fit_dt_nirpc, type = "probs"))


fit_dt_nirpc[, new_model_predict_none := P[, 'none']]
fit_dt_nirpc[, new_model_predict_mand := P[, 'mandatory']]
fit_dt_nirpc[, new_model_predict_non_mand := P[, 'non-mandatory']]
...
(day_category_without_bias_nirpc = fit_dt_nirpc[, 
                                          .(made_no_trips = sum(estimation_weight * new_model_predict_none),  
                                            made_mandatory_trips = sum(estimation_weight * new_model_predict_mand),
                                            made_nonmandatory_only = sum(estimation_weight * new_model_predict_non_mand)), 
                                          .(online_data)][order(online_data)])

(day_category_with_bias_nirpc = fit_dt_nirpc[, 
                                       .(made_no_trips = sum(estimation_weight * model_predict_none),  
                                         made_mandatory_trips = sum(estimation_weight * model_predict_mand),
                                         made_nonmandatory_only = sum(estimation_weight * model_predict_non_mand)), 
                                       .(online_data)][order(online_data)])
...
saveRDS(day_category_without_bias_nirpc, file.path(wt_dir, "model_summaries", "day_category_without_bias_nirpc.rds"))
saveRDS(day_category_with_bias_nirpc, file.path(wt_dir,"model_summaries", "day_category_with_bias_nirpc.rds"))


#
# Day pattern correction for kids based on mode to school
# # Skipping
# 
# fit_dt[, `:=` (school_mode_wb = ifelse(smode %in% 1:2, 1, 0),
#                school_mode_car = ifelse(smode %in% 3:6, 1, 0),
#                school_mode_car_schoolbus =  ifelse(smode %in% 3:7, 1, 0),
#                no_school_travel = ifelse(smode < 0 | smode == 18, 1, 0),
#                school_mode_schoolbus = ifelse(smode == 7, 1, 0)
#                               )]
# workers_adults = per_raw[, .(n_workers = sum(emply_ask == 1), n_adults = sum(age >= 18 | aage >= 5), n_kids = sum(age <18 | (aage %in%  1:5))), by = .(sampno)]
# fit_dt = workers_adults[fit_dt, on = .(sampno)]
# 
# fit_dt[, all_adults_work := ifelse(n_workers == n_adults, 1, 0)]
# fit_dt[, one_kid := ifelse(n_kids == 1, 1, 0)]
# fit_dt[, one_adult := ifelse(n_adults == 1, 1, 0)]
# fit_dt[, income_missing := ifelse(hhinc <0, 1, 0)]
# fit_dt[, grade_6_8 := ifelse(schol == 3 & (age >= 11 | aage == 3), 1, 0)]
# fit_dt[, grade_9_12 := ifelse(schol == 4, 1, 0)]
# fit_dt[, homeschool := ifelse(sloc == 1, 1, 0)]
# 
# per_raw[location_raw[home == 1], `:=` (home_lat = latitude, home_lon= longitude), on = .(sampno)]
# per_raw[location_raw[loctype == 3], `:=` (school_lat = latitude, school_lon= longitude), on = .(perno, sampno)]
# 
# 
# per_raw[, dist_to_school := get_distance_meters(c(home_lon, home_lat), c(school_lon, school_lat)) * 0.000621371]
# 
# fit_dt[per_raw, dist_to_school := i.dist_to_school, on = .(sampno, perno)]
# fit_dt[, dist_pwr_6 := dist_to_school ^ 6]
# fit_dt[is.na(dist_pwr_6), dist_pwr_6 := 0]
# fit_dt[is.na(dist_to_school), dist_to_school := 0]
# fit_dt[, dist_to_school_bin := cut(dist_to_school, c(-Inf, 0.5, 1, 2, 10, Inf))]
# 
# fit_dt[, day_school_trip_class := fcase(num_school_trips > 0, 'school-trip', 
#                                         num_trips == 0, 'no-trips',
#                                         default = 'non-school-only')]

# fit_dt_kid = copy(fit_dt[age %in%  5:17| (aage %in% 2:4)])
# 
# # the model weighted
# model_rhs = 
#   ~  
...
# 
# model = multinom(
#   update(model_rhs, day_school_trip_class ~ .),
#   data = fit_dt_kid,
#   weights = fit_dt_kid[,estimation_weight],
#   model = TRUE, Hess = TRUE)
# 
# summary(model)
# 
...
# DescTools::PseudoR2(model) # Ohio: 0.164; # Baton Rouge 0.247
# 
# saveRDS(broom::tidy(model, exponentiate = FALSE), file.path(wt_dir,"school_day_pattern_model_coef_new.rds"))
# 
# fit_dt_kid[, 
#              .(weighted_sum = sum(estimation_weight)), 
#              .(day_school_trip_class, school_mode_car, homeschool)][order(day_school_trip_class, school_mode_car, homeschool)]
# 
# # checks out
# all.equal(
#   colSums(predict(model, newdata = fit_dt_kid, type = "probs") * fit_dt_kid[, estimation_weight]),
#   fit_dt_kid[, .(weighted_sum = sum(estimation_weight)), .(day_school_trip_class)][order(day_school_trip_class), weighted_sum]
# )
# 
# fit_dt_kid[, model_predict_none := predict(model, newdata = fit_dt_kid, type = "probs")[, 'no-trips']]
# fit_dt_kid[, model_predict_school := predict(model, newdata = fit_dt_kid, type = "probs")[, 'school-trip']]
# fit_dt_kid[, model_predict_non_school := predict(model, newdata = fit_dt_kid, type = "probs")[, 'non-school-only']]
# 
# fit_dt_kid[, sum(model_predict_none * estimation_weight), by = .(homeschool, school_mode_car)]
# 
# # Recalculate predicteds without bias parameters removed
# # rMove people tend to record more trips
# 
# model_coefs = coef(model)
# 
# model_coefs[, "school_mode_car"] = 0
# model_coefs[, "homeschool"] = 0
# # model_coefs[, "online_data:age_18_35"] = 0
# 
# V_none = 0
# V_school = model.matrix(model_rhs, fit_dt_kid) %*% t(t(model_coefs['school-trip',]))
# V_non_school = model.matrix(model_rhs, fit_dt_kid) %*% t(t(model_coefs['non-school-only',]))
# 
# V = cbind(V_none=V_none, V_school=V_school, V_non_school=V_non_school)
# colnames(V) = c('none', 'school', 'non-school')
# P = exp(V)
# 
# P = P/rowSums(P)
# 
# # Expect fewer "none" trips in model with bias parameters removed.
# colSums(P)
# colSums(predict(model, newdata = fit_dt_kid, type = "probs"))
# 
# 
# fit_dt_kid[, new_model_predict_none := P[, 'none']]
# fit_dt_kid[, new_model_predict_school := P[, 'school']]
# fit_dt_kid[, new_model_predict_non_school := P[, 'non-school']]
# 
# # does it impact online only
# fit_dt_kid[, 
#              .(wbias = sum(model_predict_none * estimation_weight), 
#                wobias = sum(new_model_predict_none * estimation_weight)), 
#              by=.(school_mode_car, homeschool)][order(school_mode_car, homeschool)]
# 
# 
# (day_category_without_bias = fit_dt_kid[, 
#                                           .(made_no_trips = sum(estimation_weight * new_model_predict_none),  
#                                             made_school_trips = sum(estimation_weight * new_model_predict_school),
#                                             made_nonschool_only = sum(estimation_weight * new_model_predict_non_school)), 
#                                           .(school_mode_car, homeschool)][order(school_mode_car, homeschool)])
# 
# (day_category_with_bias = fit_dt_kid[, 
#                                        .(made_no_trips = sum(estimation_weight * model_predict_none),  
#                                          made_school_trips = sum(estimation_weight * model_predict_school),
#                                          made_nonschool_only = sum(estimation_weight * model_predict_non_school)), 
#                                        .(school_mode_car, homeschool)][order(school_mode_car, homeschool)])
# 
# 
...
targets_nirpc      = readRDS(file = file.path(wt_dir,"PUMA_2018_targets_nirpc.rds"))
survey_data  = readRDS(file = file.path(wt_dir,"p_survey.rds"))
survey_data_nirpc  = readRDS(file = file.path(wt_dir,"p_survey_nirpc.rds"))
...
person_predict_nirpc = fit_dt_nirpc[,
                                  .(p_made_no_trips = mean(new_model_predict_none),
                                    p_made_mandatory_trips = mean(new_model_predict_mand),
                                    p_made_nonmandatory_only = mean(new_model_predict_non_mand)),
                                  .(sampno, perno)]
...
person_predict_hh_nirpc = person_predict_nirpc[, 
...
survey_day_adj_nirpc = merge(survey_data_nirpc, person_predict_nirpc, by = c('sampno', 'perno'), all = TRUE)
...
survey_day_adj_nirpc[, p_made_no_trips := p_made_no_trips * (p_total - p_age_lt_5 - p_age_5_14 - p_age_15_17)]
survey_day_adj_nirpc[, p_made_mandatory_trips := p_made_mandatory_trips * (p_total - p_age_lt_5 - p_age_5_14 - p_age_15_17)]
survey_day_adj_nirpc[, p_made_nonmandatory_only  := p_made_nonmandatory_only  * (p_total - p_age_lt_5 - p_age_5_14 - p_age_15_17)]
survey_day_adj_nirpc[, p_made_not_applicable := p_age_lt_5 + p_age_5_14 + p_age_15_17]
survey_day_adj_nirpc[is.na(survey_day_adj_nirpc)]  = 0
...
saveRDS(survey_day_adj_nirpc, file.path(wt_dir, "p_survey_day_adjustments_nirpc.rds"))


cmap_weights = hh_raw[study == 'cmap', .(sampno, wthhfin = wthhfin_orig)]
nirpc_weights = hh_raw[study == 'nirpc', .(sampno,  wthhfin = wthhfin_orig)]
...
# Target data
survey_day_adj_wt_nirpc = merge(survey_day_adj_nirpc,
                          nirpc_weights[, .(sampno, wthhfin)],
                          by = 'sampno', all.x=TRUE)
...
targets_predict = 
  survey_day_adj_wt[,
                    ...
                    .(puma_id)]
targets_predict_nirpc = 
  survey_day_adj_wt_nirpc[,
                    .(p_made_no_trips          = sum(wthhfin * p_made_no_trips),
                      p_made_mandatory_trips   = sum(wthhfin * p_made_mandatory_trips),
                      p_made_nonmandatory_only = sum(wthhfin * p_made_nonmandatory_only),
                      p_made_not_applicable    = sum(wthhfin * (p_age_lt_5+ p_age_5_14 + p_age_15_17))),
...
targets_predict_nirpc[, puma_id := str_pad(puma_id, 5, pad = '0')]
...
targets_day_adjustment_nirpc = merge(targets_predict_nirpc[, c('puma_id', cols), with = FALSE], targets_nirpc, by = 'puma_id', all.x=TRUE)
...
targets_day_adjustment_nirpc[, scale := sum(p_total - p_made_not_applicable) /
                         sum(p_made_no_trips + p_made_mandatory_trips + p_made_nonmandatory_only), .(puma_id)]

targets_day_adjustment_nirpc[, p_made_no_trips := scale * p_made_no_trips]
targets_day_adjustment_nirpc[, p_made_mandatory_trips := scale * p_made_mandatory_trips]
targets_day_adjustment_nirpc[, p_made_nonmandatory_only := scale * p_made_nonmandatory_only]
...
saveRDS(targets_day_adjustment_nirpc,
        file.path(wt_dir, "PUMA_2018_targets_day_adjustments_nirpc.rds"))
