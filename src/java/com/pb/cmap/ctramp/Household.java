...
public class Household implements HouseholdIf, java.io.Serializable
{
    ...
    public int getNumberOfChildrenUnder16WithHomeOrNonMandatoryActivity() {
        ...
        for (int i = 1; i < persons.length; ++i) {
            if ( persons[i].getAge() < 16 && (! persons[i].getCdapActivity().equalsIgnoreCase(ModelStructure.MANDATORY_PATTERN)) )
                count ++;
        }
    ...
    public int getNumberOfDrivingAgedStudentsWithDcPurposeIndex(int segmentIndex)
    {
        ...
        for (int j = 1; j < persons.length; j++)
        {
            if (persons[j].getPersonIsStudentDriving() == 1
                    && persons[j].getSchoolLocationPurposeIndex() == segmentIndex) num++;
        }
        return num;
    }
    ...
    {
        return modelStructure.getIncomeSegment(hhIncomeInDollars);
    }
    ...
    {
        int numChildrenUnder18 = 0;
        for (int i = 1; i < persons.length; i++)
        {
            if (persons[i].getAge() < 18) numChildrenUnder18++;
        }
    ...
    public Person getOldestPerson()
    {
        ...
        Person oldestPerson = persons[1];
        int maxAge = persons[1].getAge();
    ...
    public int getHighestEducationAttainment()
    {
        ...
        int educ = 0;
        for (int i = 1; i < persons.length; i++)
        	educ = persons[i].getPersonEducAttainment();
        	highest = (educ > highest) ? educ : highest; 
        return highest;
    }
            ...
            for (int q = 1; q < persons.length; q++)
            {
                ...
                if ( q > p ){
                    if (pAdult && qAdult) {
                        ...
                        if (maxWindow > maxAdultChildOverlaps)
                            maxAdultChildOverlaps = maxWindow;
                    }
                }
    ...
    public boolean[] getAvailableJointTourTimeWindows(TourIf t, int[] altStarts, int[] altEnds)
    {
    ...
    public int getMaxJointTimeWindow( TourIf t )
    {
        ...
        byte[][] personWindows = new byte[participatingPersonIndices.length][]; 
    ...
    public void scheduleJointTourTimeWindows(TourIf t, int start, int end)
    {
        byte[] participatingPersonIndices = t.getPersonNumArray(); 
        for (int i : participatingPersonIndices )            
        {
    ...
    // this constructor used for joint tour creation for household with 1 joint tour
    public void createJointTourArray(Object[] t1Data)
    {
        int primaryPurposeIndex = modelStructure.getPrimaryPurposeIndex( modelStructure.getPrimaryPurposeString((String)t1Data[1]) );
        Tour t1 = new Tour( this, (String) t1Data[1], ModelStructure.JOINT_NON_MANDATORY_CATEGORY, primaryPurposeIndex );
        
        jointTours = new Tour[1];
        t1.setTourOrigTaz(homeTaz);
        t1.setTourDestTaz(0);
        jointTours[0] = t1;
    }

    // this constructor used for joint tours creation for household with 2 joint tours
    public void createJointTourArray(Object[] t1Data, Object[] t2Data)
    {
        int primaryPurposeIndex = modelStructure.getPrimaryPurposeIndex( modelStructure.getPrimaryPurposeString((String)t1Data[1]) );
        Tour t1 = new Tour( this, (String) t1Data[1], ModelStructure.JOINT_NON_MANDATORY_CATEGORY, primaryPurposeIndex );

        primaryPurposeIndex = modelStructure.getPrimaryPurposeIndex( modelStructure.getPrimaryPurposeString((String)t2Data[1]) );
        Tour t2 = new Tour( this, (String) t2Data[1], ModelStructure.JOINT_NON_MANDATORY_CATEGORY, primaryPurposeIndex );
        
        jointTours = new Tour[2];
        t1.setTourOrigTaz(homeTaz);
        t1.setTourDestTaz(0);
        t1.setTourId(0);
        t2.setTourOrigTaz(homeTaz);
        t2.setTourDestTaz(0);
        t2.setTourId(1);
        jointTours[0] = t1;
        jointTours[1] = t2;
    ...
    public void logHouseholdObject(String titleString, Logger logger)
    {
        ...
        Household.logHelper(logger, "immcRandomCount: ", immcRandomCount, totalChars);
        ...
        Household.logHelper(logger, "awmcRandomCount: ", awmcRandomCount, totalChars);
        ...
        Household.logHelper(logger, "jmcRandomCount: ", jmcRandomCount, totalChars);
        ...
        Household.logHelper(logger, "inmmcRandomCount: ", inmmcRandomCount, totalChars);
    ...
    public void logPersonObject(String titleString, Logger logger, PersonIf person)
    {
    ...
    public void logTourObject(String titleString, Logger logger, PersonIf person, TourIf tour)
    {
    ...
    public void logStopObject( String titleString, Logger logger, StopIf stop, ModelStructure modelStructure ) {
    ...
    public long getHouseholdObjectSize() {
        long size = 0;
        try{
            size = ObjectUtil.checkObjectSize( this );                        
        }
        catch ( Exception e ){
        }
        return size;
