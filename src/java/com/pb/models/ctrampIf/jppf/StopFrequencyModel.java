...
public class StopFrequencyModel implements Serializable {
    ...
    public void applyModel( HouseholdIf household ){
        ...
        if ( jt != null ) {
            ...
            int tourCount = 0;
            for ( TourIf tour : tourList ) {

                try {

                    int modelIndex = modelStructure.getStopFrequencyModelIndex( tour.getTourPrimaryPurpose().toLowerCase() );
                    ...
                    if( household.getDebugChoiceModels() ) {
                        ...
                        decisionMakerLabel = String.format ( "HH=%d, TourType=%s, TourId=%d, TourPurpose=%s.", household.getHhId(), ModelStructure.TOUR_CATEGORY_LABELS[tour.getTourCategoryIndex()], tour.getTourId(), tour.getTourPrimaryPurpose() );
            ...
            for ( TourIf tour : tourList ) {
                ...
                try {
                    ...
                    if( household.getDebugChoiceModels() ) {
                        ...
                        decisionMakerLabel = String.format ( "HH=%d, PersonNum=%d, PersonType=%s, TourType=%s, TourId=%d, TourPurpose=%s.", household.getHhId(), person.getPersonNum(), person.getPersonType(), ModelStructure.TOUR_CATEGORY_LABELS[tour.getTourCategoryIndex()], tour.getTourId(), tour.getTourPrimaryPurpose() );
    ...
    private int setStopFreqChoice ( TourIf tour, int stopFreqChoice ) {

        tour.setStopFreqChoice ( stopFreqChoice );


        // set argument values for method call to get stop purpose
        HouseholdIf hh = tour.getPersonObject().getHouseholdObject();
    ...
    private void readPurposeLookupProportionsTable( String purposeLookupFilename ) {
        ...
        int start = modelStructure.getTimePeriodIndexForTime( ModelStructure.FIRST_TOD_INTERVAL_HOUR );
        int end = modelStructure.getTimePeriodIndexForTime( ModelStructure.LAST_TOD_INTERVAL_HOUR );
        int numDepartHours = end + 1;
        ...
        personTypeMap.put( FT_WORKER_PERSON_TYPE_NAME, PersonIf.PERSON_TYPE_FULL_TIME_WORKER_NAME );
        personTypeMap.put( PT_WORKER_PERSON_TYPE_NAME, PersonIf.PERSON_TYPE_PART_TIME_WORKER_NAME );
        personTypeMap.put( UNIVERSITY_PERSON_TYPE_NAME, PersonIf.PERSON_TYPE_UNIVERSITY_STUDENT_NAME );
        personTypeMap.put( NONWORKER_PERSON_TYPE_NAME, PersonIf.PERSON_TYPE_NON_WORKER_NAME );
        personTypeMap.put( RETIRED_PERSON_TYPE_NAME, PersonIf.PERSON_TYPE_RETIRED_NAME );
        personTypeMap.put( DRIVING_STUDENT_PERSON_TYPE_NAME, PersonIf.PERSON_TYPE_STUDENT_DRIVING_NAME );
        personTypeMap.put( NONDRIVING_STUDENT_PERSON_TYPE_NAME, PersonIf.PERSON_TYPE_STUDENT_NON_DRIVING_NAME );
        personTypeMap.put( PRESCHOOL_PERSON_TYPE_NAME, PersonIf.PERSON_TYPE_PRE_SCHOOL_CHILD_NAME );
        ...
        for ( int i=0; i < purposeLookupTable.getRowCount(); i++ ) {
            ...
            int startInterval = modelStructure.getTimePeriodIndexForTime( startRange );
            ...
            int endInterval = modelStructure.getTimePeriodIndexForTime( endRange );
    ...
    private int getStopPurpose ( HouseholdIf household, String halfTourDirection, int todInterval, String tourPrimaryPurpose, String personType ) {
        
        double[] props = null;
        String key = tourPrimaryPurpose + "_" + personType;
        if ( halfTourDirection.equalsIgnoreCase( OUTBOUND_DIRECTION_NAME ) )
            props = outProportionsMaps[todInterval].get(key);
        else if ( halfTourDirection.equalsIgnoreCase( INBOUND_DIRECTION_NAME ) )
            props = inProportionsMaps[todInterval].get(key);

        double rn = household.getHhRandom().nextDouble();
        int choice = ChoiceModelApplication.getMonteCarloSelection( props, rn );
        
        // props is 0 based, so add 1 to the choice to get the true purpose index
        return choice + 1;
