...
public class TazDataHandler implements TazDataIf, Serializable {
    ...
    public static final String PROPERTIES_TAZ_DATA_FIELD_NAME          = "TazData.File.Field.Name";
    public static final String PROPERTIES_ZONAL_ACCESSIBILITIESS_DATA  = "ZonalAccessibilities.file";
    public static final String PROPERTIES_ZONAL_ACCESSIBILITIESS_DATA_FIELD_NAME  = "ZonalAccessibilities.Field.Name";

    // default values - can be overridden by subclass setter methods
    private static final String ZONE_DATA_ZONE_FIELD_NAME = "maz";
    ...
    private static final String ZONE_DATA_BHHDEN_FIELD_NAME = "b_hhden";
    private static final String ZONE_DATA_BEMPDEN_FIELD_NAME = "b_empden";
    private static final String ZONE_DATA_PEF_FIELD_NAME = "pef";
    private static final String ZONE_DATA_RING_FIELD_NAME = "ring";

    private static final String ACCESSIBILITIES_FILE_ZONE_FIELD_NAME = "maz";
    ...
    private static final String ACCESSIBILITIES_17_FIELD_NAME = "access17";
    private static final String ACCESSIBILITIES_18_FIELD_NAME = "access18";

    private static final String[] SUB_ZONE_NAMES = { "all"}; //1 subzone now
    ...
    private float[] bhhden;
    private float[] bempden;
    private float[] pef;
    private int[] ring;
    ...
    protected String tazDataGradeSchoolFieldName= "";
    protected String tazDataHighSchoolFieldName= "";
    protected String bhhdenFieldName = "";
    protected String bempdenFieldName = "";
    protected String pefFieldName = "";
    protected String ringFieldName = "";


    protected String zonalAccessibilityFileName;
    protected String zonalAccessibilityFieldName;
    ...
    // 1st dimension is indexed by zone number, 2nd dimension is walk segments - 1 walk segment now
    ...
    public TazDataHandler( ResourceBundle rb, String projectDirectory ){
        ...
        bhhdenFieldName = ZONE_DATA_BHHDEN_FIELD_NAME;
        bempdenFieldName = ZONE_DATA_BEMPDEN_FIELD_NAME;
        pefFieldName = ZONE_DATA_PEF_FIELD_NAME;
        ringFieldName = ZONE_DATA_RING_FIELD_NAME;
        ...
        tazDataZoneFieldName = ResourceUtil.getProperty(rb, PROPERTIES_TAZ_DATA_FIELD_NAME);
        ...
        zonalAccessibilityFieldName = ResourceUtil.getProperty(rb, PROPERTIES_ZONAL_ACCESSIBILITIESS_DATA_FIELD_NAME);
    ...
    public void setupTazDataManager() {
		...
		// prepare the zonal walk percent array - 1 subzone type now 100% coverage
    ...
    private void readZonalAccessibilitiesFile (String fileName) {
        ...
        access17 = new float[NUM_ZONES + 1];
        access18 = new float[NUM_ZONES + 1];
        ...
        if (fileName != null) {
            ...
            try {
                ...
                int tazPosition = acc.getColumnPosition( zonalAccessibilityFieldName );
                if (tazPosition <= 0) {
                    logger.fatal( String.format("expected zone field name=%s was not a field in the zonal accessibilities file: %s.", zonalAccessibilityFieldName, fileName));
                    throw new RuntimeException();
                }
                ...
                int access17Position = acc.getColumnPosition( ACCESSIBILITIES_17_FIELD_NAME );
                if (access17Position <= 0) {
                    logger.fatal( String.format("expected field name=%s was not a field in the zonal accessibilities file: %s.", ACCESSIBILITIES_17_FIELD_NAME, fileName));
                    throw new RuntimeException();
                }
                
                int access18Position = acc.getColumnPosition( ACCESSIBILITIES_18_FIELD_NAME );
                if (access18Position <= 0) {
                    logger.fatal( String.format("expected field name=%s was not a field in the zonal accessibilities file: %s.", ACCESSIBILITIES_18_FIELD_NAME, fileName));
                    throw new RuntimeException();
                }
                ...
                for (int i=1; i <= acc.getRowCount(); i++) {
                    ...
                    access17[taz] = acc.getValueAt(i, access17Position);
                    access18[taz] = acc.getValueAt(i, access18Position);
                }
   ...
   private int getZoneFromAlt ( int alt ) {
    ...
    /**
     * @return gradeschool array from the zone data table.
     */
    public int[] getZonalGradeSchool () {
        int gradeSchoolFieldPosition = zoneDataTable.getColumnPosition( tazDataGradeSchoolFieldName );
        if ( gradeSchoolFieldPosition < 0 ) {
            logger.error ( String.format("The gradeSchool field name = %s defined in %s is not found as a field name in the zone data table.", tazDataGradeSchoolFieldName, this.getClass().getName() ));
            throw new RuntimeException();
        }
        return zoneDataTable.getColumnAsInt( gradeSchoolFieldPosition );
    }
    
    /**
     * @return highschool array from the zone data table.
     */
    public int[] getZonalHighSchool () {
        int highschoolFieldPosition = zoneDataTable.getColumnPosition( tazDataHighSchoolFieldName );
        if ( highschoolFieldPosition < 0 ) {
            logger.error ( String.format("The highschool field name = %s defined in %s is not found as a field name in the zone data table.", tazDataHighSchoolFieldName, this.getClass().getName() ));
            throw new RuntimeException();
        }
        return zoneDataTable.getColumnAsInt( highschoolFieldPosition );
    }
    
    /**
     * @return bhhden array from the zone data table.
     */
    public float[] getBHHDen () {
        int bhhdenFieldPosition = zoneDataTable.getColumnPosition( bhhdenFieldName );
        if ( bhhdenFieldPosition < 0 ) {
            logger.error ( String.format("The bhhden field name = %s defined in %s is not found as a field name in the zone data table.", bhhdenFieldName, this.getClass().getName() ));
            throw new RuntimeException();
        }
        return zoneDataTable.getColumnAsFloat( bhhdenFieldPosition );
    }
    
    /**
     * @return bempden array from the zone data table.
     */
    public float[] getBEmpDen () {
        int bempdenFieldPosition = zoneDataTable.getColumnPosition( bempdenFieldName );
        if ( bempdenFieldPosition < 0 ) {
            logger.error ( String.format("The bempden field name = %s defined in %s is not found as a field name in the zone data table.", bempdenFieldName, this.getClass().getName() ));
            throw new RuntimeException();
        }
        return zoneDataTable.getColumnAsFloat( bempdenFieldPosition );
    }
    
    /**
     * @return pef array from the zone data table.
     */
    public float[] getPef () {
        int pefFieldPosition = zoneDataTable.getColumnPosition( pefFieldName );
        if ( pefFieldPosition < 0 ) {
            logger.error ( String.format("The pef field name = %s defined in %s is not found as a field name in the zone data table.", pefFieldName, this.getClass().getName() ));
            throw new RuntimeException();
        }
        return zoneDataTable.getColumnAsFloat( pefFieldPosition );
    }
    
    /**
     * @return ring array from the zone data table.
     */
    public int[] getRing () {
        int ringFieldPosition = zoneDataTable.getColumnPosition( ringFieldName );
        if ( ringFieldPosition < 0 ) {
            logger.error ( String.format("The ring field name = %s defined in %s is not found as a field name in the zone data table.", ringFieldName, this.getClass().getName() ));
            throw new RuntimeException();
        }
        return zoneDataTable.getColumnAsInt( ringFieldPosition );
    }
    ...
    public float[] getAccess17Accessibity() {
        return access17;
    }

    public float[] getAccess18Accessibity() {
        return access18;
    }
    ...
    public MazTapTazData getMttData() {
    	MazTapTazData mttData = null;
    	try {
    		mttData = new MazTapTazData(null);
    	} catch (IOException e) {  
