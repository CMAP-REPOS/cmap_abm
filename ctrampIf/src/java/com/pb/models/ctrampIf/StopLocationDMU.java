...
public class StopLocationDMU implements Serializable, VariableTable {
    ...
    public HouseholdIf household;
    public PersonIf person;
    public TourIf tour;
    public StopIf stop;
    ...
    public void setHouseholdObject ( HouseholdIf household) {
        this.household = household;
    }

    public void setPersonObject ( PersonIf person ) {
        this.person = person;
    }

    public void setTourObject(TourIf tour) {
        this.tour = tour;
    }

    public void setStopObject(StopIf stop) {
        this.stop = stop;
    }
    ...
    public int getStopPurposeIsWork() {
        return stop.getDestPurpose().equalsIgnoreCase( ModelStructure.WORK_PURPOSE_NAME ) ? 1 : 0;
    }

    public int getStopPurposeIsEscort() {
        return stop.getDestPurpose().equalsIgnoreCase( ModelStructure.ESCORT_PURPOSE_NAME ) ? 1 : 0;
    }

    public int getStopPurposeIsShopping() {
        return stop.getDestPurpose().equalsIgnoreCase( ModelStructure.SHOPPING_PURPOSE_NAME ) ? 1 : 0;
    }

    public int getStopPurposeIsEatOut() {
        return stop.getDestPurpose().equalsIgnoreCase( ModelStructure.EAT_OUT_PURPOSE_NAME ) ? 1 : 0;
    }

    public int getStopPurposeIsOthMaint() {
        return stop.getDestPurpose().equalsIgnoreCase( ModelStructure.OTH_MAINT_PURPOSE_NAME ) ? 1 : 0;
    }

    public int getStopPurposeIsSocial() {
        return stop.getDestPurpose().equalsIgnoreCase( ModelStructure.SOCIAL_PURPOSE_NAME ) ? 1 : 0;
    }

    public int getStopPurposeIsOthDiscr() {
        return stop.getDestPurpose().equalsIgnoreCase( ModelStructure.OTH_DISCR_PURPOSE_NAME ) ? 1 : 0;
    }
