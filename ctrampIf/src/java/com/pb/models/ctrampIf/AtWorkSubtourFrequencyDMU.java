...
public class AtWorkSubtourFrequencyDMU implements Serializable, VariableTable {
    ...
    public void setPersonObject ( PersonIf persObject ) {
        person = persObject;
    }
    ...
    public int getWorkTourDuration() {
        int start = tour.getTourDepartPeriod();
        int end = tour.getTourArrivePeriod();
        return end - start;
    }
    ...
    public int getIndivEatOut() {
        int numTours = 0;
        ArrayList<TourIf> tourList = person.getListOfIndividualNonMandatoryTours();
        if ( tourList != null ) {
