...
public class CoordinatedDailyActivityPatternModel {
	...
	protected PersonIf[] cdapPersonArray;
    ...
    public void saveResults(HouseholdDataManagerIf householdDataManager, String projectDirectory){
        ...
        if ( cdapResultsFileName != null ) {
            ...
            HouseholdIf[] householdArray = householdDataManager.getHhArray();
        ...
        for(int i=1; i < householdArray.length; ++i){
            ...
            for(int j=1;j<personArray.length;++j){
                ...
                if(personArray[j].getPersonTypeIsFullTimeWorker()==1){
                    personTypeString = PersonIf.PERSON_TYPE_FULL_TIME_WORKER_NAME;
                }

                else if(personArray[j].getPersonTypeIsPartTimeWorker()==1){
                    personTypeString = PersonIf.PERSON_TYPE_PART_TIME_WORKER_NAME;
                }

                else if(personArray[j].getPersonIsUniversityStudent()==1){
                    personTypeString = PersonIf.PERSON_TYPE_UNIVERSITY_STUDENT_NAME;
                }

                else if(personArray[j].getPersonIsNonWorkingAdultUnder65()==1){
                    personTypeString = PersonIf.PERSON_TYPE_NON_WORKER_NAME;
                }

                else if(personArray[j].getPersonIsNonWorkingAdultOver65()==1){
                    personTypeString = PersonIf.PERSON_TYPE_RETIRED_NAME;
                }

                else if(personArray[j].getPersonIsStudentDriving()==1){
                    personTypeString = PersonIf.PERSON_TYPE_STUDENT_DRIVING_NAME;
                }

                else if(personArray[j].getPersonIsStudentNonDriving()==1){
                    personTypeString = PersonIf.PERSON_TYPE_STUDENT_NON_DRIVING_NAME;
                }

                else if(personArray[j].getPersonIsPreschoolChild()==1){
                    personTypeString = PersonIf.PERSON_TYPE_PRE_SCHOOL_CHILD_NAME;
                }
     ...
     */
	public String getCoordinatedDailyActivityPatternChoice( HouseholdIf householdObject ){
		...
		for(int i=0;i<modelHhSize;++i){
			...
			// get personA
			PersonIf personA = getCdapPerson(i+1);
			...
			for(int j=0;j<modelHhSize;++j){
				...
				PersonIf personB = getCdapPerson(j+1);
				...
				for(int k=0;k<modelHhSize;++k){
					...
					PersonIf personC = getCdapPerson(k+1);
	 ...
	 * 
	 */
	private String applyModelForExtraHhMembers(HouseholdIf householdObject, String patternStringForOtherHhMembers){
		
		String allMembersPattern = patternStringForOtherHhMembers;
		
		// get the persons not yet modeled
		PersonIf[] personArray = getPersonsNotModeledByCdap(MAX_MODEL_HH_SIZE);
	...
	private void setUpModel(String uecFileName, ResourceBundle resourceBundle){
		...
		onePersonUec = new UtilityExpressionCalculator(new File(uecFileName),UEC_ONE_PERSON,UEC_DATA_PAGE,propertyMap,(VariableTable)dmuObject);
		twoPeopleUec = new UtilityExpressionCalculator(new File(uecFileName),UEC_TWO_PERSON,UEC_DATA_PAGE,propertyMap,(VariableTable)dmuObject);
		threePeopleUec = new UtilityExpressionCalculator(new File(uecFileName),UEC_THREE_PERSON,UEC_DATA_PAGE,propertyMap,(VariableTable)dmuObject);
		allMemberInteractionUec = new UtilityExpressionCalculator(new File(uecFileName),UEC_ALL_PERSON,UEC_DATA_PAGE,propertyMap,(VariableTable)dmuObject);
     ...
     *
     */
    public void reOrderPersonsForCdap(HouseholdIf household){
    	...
    	cdapPersonArray = new PersonIf[persons.length];
    ...
    protected PersonIf getCdapPerson(int persNum){
     ...
     * @return
     */
    public PersonIf[] getPersonsNotModeledByCdap(int personsModeledByCdap){

    	// create a 1-based person array to be consistent
    	PersonIf[] personArray = new PersonIf[cdapPersonArray.length - personsModeledByCdap];
