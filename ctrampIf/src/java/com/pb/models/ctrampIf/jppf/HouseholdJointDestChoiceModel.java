...
import com.pb.models.ctrampIf.jppf.ModeChoiceModel;
import com.pb.common.newmodel.ChoiceModelApplication;
import org.apache.log4j.Logger;



public class HouseholdJointDestChoiceModel implements Serializable {

    private transient Logger logger = Logger.getLogger(HouseholdJointDestChoiceModel.class);
    ...
    private static final String PROPERTIES_JOINT_SOA_SAMPLE_SIZE_KEY = "JointTourLocationChoice.SampleOfAlternatives.SampleSize";
...
//    public HouseholdJointDestChoiceModel( HashMap<String, String> propertyMap, ModelStructure modelStructure, TazDataIf tazDataManager, CtrampDmuFactoryIf dmuFactory, ModeChoiceModel mcModel, double[][][] jointProbabilitiesCache, double[][][] jointCumProbabilitiesCache ){
      public HouseholdJointDestChoiceModel( HashMap<String, String> propertyMap, ModelStructure modelStructure, TazDataIf tazDataManager, CtrampDmuFactoryIf dmuFactory, ModeChoiceModel mcModel ){

        
        this.tourCategory = ModelStructure.JOINT_NON_MANDATORY_CATEGORY;
        ...
        //setupDestChoiceModelArrays( propertyMap, tazDataManager, dmuFactory, jointProbabilitiesCache, jointCumProbabilitiesCache );
      ...
      //private void setupDestChoiceModelArrays( HashMap<String, String> propertyMap, TazDataIf tazDataManager, CtrampDmuFactoryIf dmuFactory, double[][][] jointProbabilitiesCache, double[][][] jointCumProbabilitiesCache ) {
      private void setupDestChoiceModelArrays( HashMap<String, String> propertyMap, TazDataIf tazDataManager, CtrampDmuFactoryIf dmuFactory ) {
        ...
        //System.out.println( "before new DestChoiceSize() " + (time2 - time1) );
        DestChoiceSize dcSizeObj = new DestChoiceSize( modelStructure, tazDataManager );

        //long time3 = System.currentTimeMillis();
        //System.out.println( "before setupDestChoiceSize() " + (time3 - time2) );
        dcSizeObj.setupDestChoiceSize( propertyMap, projectDirectory, tourCategory );

        //long time4 = System.currentTimeMillis();
        //System.out.println( "before calculateDcSize() " + (time4 - time3) );
    ...
    public void applyModel( HouseholdIf hh ) {
        ...
        if ( hh.getDebugChoiceModels() )
            hh.logHouseholdObject( "Pre Joint Tour Location Choice HHID=" + hh.getHhId() + " Object", modelLogger );
        


        TourIf[] jt = hh.getJointTourArray();
        if ( jt == null )
            return;
        ...
        for ( i=0; i < jt.length; i++ ) {
            ...
            // get a person object to use for the choice model - use first person added to joint tour
            byte[] personsInTour = tour.getPersonNumArray();
            int personNum = personsInTour[0];
            PersonIf p = hh.getPersons()[personNum];
            ...
            try {
                ...
                mcDmuObject.setDmuIndexValues( hh.getHhId(), homeTaz, origTaz, 0, hh.getDebugChoiceModels() );

                // at destination choice stage for home based joint non-mandatory tours, time of day has not been selected,
                // so use default mid-day to compute ms logsum.
                mcDmuObject.setTourDepartPeriod( modelStructure.getNonMandatoryLocationDefaultDepartPeriod() );
                mcDmuObject.setTourArrivePeriod( modelStructure.getNonMandatoryLocationDefaultArrivePeriod() );
                ...
                logger.fatal( String.format("exception caught selecting %s tour destination choice for hh.hhid=%d, personNum=%d, i=%d, purposeIndex=%d, purposeName=%s", tourCategory, hh.getHhId(), personNum, i, purposeIndex, purposeName ) );
            ...
            tour.setTourDepartPeriod( 0 );
            tour.setTourArrivePeriod( 0 );
    ...
    private int selectLocationFromSampleOfAlternatives( DestChoiceDMU dcDmuObject, DcSoaDMU dcSoaDmuObject, TourModeChoiceDMU mcDmuObject, TourIf tour, PersonIf person, String purposeName, int purposeIndex, int choiceNum ) {

        int uecIndex = modelStructure.getDcUecIndexForPurpose( purposeName );

        HouseholdIf household = dcDmuObject.getHouseholdObject();
        ...
        int logsumIndex = modelStructure.getPeriodCombinationIndex( tour.getTourDepartPeriod(), tour.getTourArrivePeriod() );

        Logger modelLogger = dcNonManLogger;
        String choiceModelDescription = String.format ( "Joint Tour Mode Choice Logsum calculation for %s Location Choice", purposeName );
        ...
        for (int i=1; i <= numUniqueAltsInSample; i++) {
            ...
            double logsum = mcModel.getModeChoiceLogsum( mcDmuObject, tour.getTourPrimaryPurpose(), modelLogger, choiceModelDescription, decisionMakerLabel );
        ...
        if ( household.getDebugChoiceModels() ) {
            ...
            choiceModelDescription = String.format ( "Joint Tour Destination Choice Model, Purpose=%s", purposeName );
    ...
    private int getSoaSampleSize( HashMap<String, String> propertyMap ) {
        ...
        propertyValue = propertyMap.get( PROPERTIES_JOINT_SOA_SAMPLE_SIZE_KEY );
