...
public class HouseholdChoiceModelsTaskJppf<T> extends AbstractTask<T> {
    ...
    public void run() {
        ...
        try {
            ...
            this.tazDataManager = (TazDataIf) dataProvider.getParameter("tazDataManager");
            this.hhDataManager = (HouseholdDataManagerIf) dataProvider.getParameter("householdDataManager");
        ...
        HouseholdIf[] householdArray = hhDataManager.getHhArray( startIndex, endIndex ); 
        ...
        setResult( (T)String.format("taskIndex=%d, hhModelInstance=%d, startIndex=%d, endIndex=%d, thread=%s", taskIndex, hhModel.getModelIndex(), startIndex, endIndex, threadName) );
    ...
    private void logModelComponentTimes( long[] componentTimes, long[] partialStopTimes, Logger logger, int modelIndex ) {
        ...
        String[] label1 = { "AO", "FP", "CDAP", "IMTF", "IMTOD", "IMMC", "JTF", "JTDC", "JTTOD", "JTMC", "INMTF", "INMTDC", "INMTTOD", "INMTMC", "AWTF", "AWTDC", "AWTTOD", "AWTMC", "STF", "STDTM" };
        ...
        for ( int i=0; i < componentTimes.length; i++ ) {
            ...
            logger.info( String.format("%-6d%24s:%15.1f", (i+1), label1[i], time ) );
            total += time;
        }       
        logger.info( String.format("%-6s%24s:%10.1f", "Total", "Total all components", total ) );
        logger.info( "" );
        
        String[] label2 = { "SLC SOA", "SLC LS", "SLC", "SLC TOT", "S TOD", "S MC", "S PLC", "TOTAL" };
        ...
        for ( int i=0; i < partialStopTimes.length; i++ ) {
            ...
            logger.info( String.format("%-6d%24s:%15.1f", (i+1), label2[i], time ) );
        }       
