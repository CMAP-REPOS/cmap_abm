...
import com.pb.models.ctrampIf.jppf.DestinationSampleOfAlternativesModel;
...
//        coeffCount = new long[mcModel.getCoeffCount().length][mcModel.getCoeffCount()[0].length];
    ...
    public void applyWorkSchoolLocationChoice( HouseholdIf hh ) {
...
//        }
        ...
        for ( i=1; i < persons.length; i++ ) {
            ...
            if ( p.getPersonIsPreschoolChild() == 1 || p.getPersonIsStudentNonDriving() == 1 || p.getPersonIsStudentDriving() == 1 ) {
                ...
                if ( runWorkTours && p.getPersonIsStudentDriving() == 1 && p.getPersonIsWorker() == 1 )
                    dcPurposeList.add( modelStructure.getWorkPurposeFromOccupation( p.getPersonWorkerOccupation() ));
            }
            ...
            for ( String dcPurposeName : dcPurposeList ) {
                ...
                try {
                    ...
                    mcDmuObject.setTourObject( p.makeDefaultTour( modelStructure.getPrimaryPurposeIndex( modelStructure.getPrimaryPurposeString(purposeName) ) ) ); // make a Tour object for use by the MC DMU for work/school location choice MC logsum
                    mcDmuObject.setDmuIndexValues( hh.getHhId(), hh.getHhTaz(), origTaz, 0, hh.getDebugChoiceModels() );
    ...
    private int selectLocationFromSampleOfAlternatives( DestChoiceDMU dcDmuObject, DcSoaDMU dcSoaDmuObject, TourModeChoiceDMU mcDmuObject, TourIf tour, PersonIf person, String purposeName, int purposeIndex, int choiceNum ) {
        ...
        int tourArrive = -1; 
        Logger modelLogger = null;
        if ( modelStructure.getPrimaryPurposeString(purposeName).equalsIgnoreCase( ModelStructure.WORK_PRIMARY_PURPOSE_NAME ) ) {
            tourPrimaryPurpose = ModelStructure.WORK_PRIMARY_PURPOSE_NAME;
            tourDepart = modelStructure.getWorkLocationDefaultDepartPeriod();
            tourArrive = modelStructure.getWorkLocationDefaultArrivePeriod();
            origTaz = household.getHhTaz();
            origSubz = household.getHhWalkSubzone();
            modelLogger = dcManLogger;
        }
        else if ( modelStructure.getPrimaryPurposeString(purposeName).equalsIgnoreCase( ModelStructure.UNIVERSITY_PURPOSE_NAME ) ) {
            tourPrimaryPurpose = ModelStructure.UNIVERSITY_PRIMARY_PURPOSE_NAME;
        ...
        else if ( modelStructure.getPrimaryPurposeString(purposeName).equalsIgnoreCase( ModelStructure.SCHOOL_PURPOSE_NAME ) ) {
            tourPrimaryPurpose = ModelStructure.SCHOOL_PRIMARY_PURPOSE_NAME;
        ...
        int logsumIndex = modelStructure.getPeriodCombinationIndex( tourDepart, tourArrive );
        ...
        if ( household.getDebugChoiceModels() ) {
            choiceModelDescription = String.format ( "Tour Mode Choice Logsum calculation for %s Location Choice", tourPrimaryPurpose );
        ...
        for (int i=1; i <= numUniqueAltsInSample; i++) {
            ...
            double logsum = mcModel.getModeChoiceLogsum( mcDmuObject, tourPrimaryPurpose, modelLogger, choiceModelDescription, decisionMakerLabel );
...
//                    coeffCount[j][k] += counts[j][k];
        ...
        if ( household.getDebugChoiceModels() ) {
            ...
            choiceModelDescription = String.format ( "Usual Location Choice Model for: Purpose=%s", tourPrimaryPurpose );
            ...
            modelLogger.info(" ");
            modelLogger.info("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
            modelLogger.info( "Usual Location Choice Model for: Purpose=" + tourPrimaryPurpose +
        ...
        if ( dcModel[uecIndex].getAvailabilityCount() > 0 ) {
            ...
            logger.error ( String.format( "Exception caught for HHID=%d, PersonNum=%d, no available %s destination choice alternatives to choose from in choiceModelApplication.", dcDmuObject.getHouseholdObject().getHhId(), dcDmuObject.getPersonObject().getPersonNum(), tourPrimaryPurpose ) );
        }
