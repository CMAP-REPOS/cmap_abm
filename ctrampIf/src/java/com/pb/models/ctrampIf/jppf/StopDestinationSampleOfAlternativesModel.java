...
public class StopDestinationSampleOfAlternativesModel implements Serializable {
    ...
    private Map<String,Integer> uecPageMap;
...
//    }
    ...
    private String[] getMainAndSubPurposeFromLookupIndex(int lookupIndex) {
        ...
        for (String purpose : lookupIndexMap.keySet()) {
            if (lookupIndexMap.get(purpose) == purposeIndex) {
                if (purpose.equals(ModelStructure.ESCORT_PURPOSE_NAME) & modelStructure.getNumEscortSegments() > 0)
    ...
    public class StopSoaResult implements Serializable {
        ...
        private void setNumUniqueAltsInSample( int numAlts ){
            this.numUniqueAlts = numAlts;
        }
        private void setStopLocationSize( double[] stopLocSize ){
    ...
    public StopSoaResult computeDestinationSampleOfAlternatives( StopIf s ) {
        ...
        HouseholdIf hh = t.getPersonObject().getHouseholdObject();
