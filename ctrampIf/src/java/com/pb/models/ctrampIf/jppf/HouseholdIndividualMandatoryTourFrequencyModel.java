...
public class HouseholdIndividualMandatoryTourFrequencyModel implements Serializable {
		...
		for(int j=1;j<personArray.length;++j){
            ...
            try {
                ...
                if( person.getPersonIsPreschoolChild() == 0 && activity.equalsIgnoreCase(MANDATORY_ACTIVITY) ){
                    ...
                    //get work purpose if worker
                    if(person.getPersonIsWorker() == 1) {
                        workPurpose = modelStructure.getWorkPurposeFromOccupation( person.getPersonWorkerOccupation() );
                    }
	...
	private void logPersonChoiceCalculations( HouseholdIf household, PersonIf person, int chosenAlternative, double rn ) {
