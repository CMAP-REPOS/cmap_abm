...
public class UtilRmi implements java.io.Serializable {
    ...
    private static final int VERSION_TRACKING = 1;
    ...
    public Object method( String name, Object[] args ) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger(UtilRmi.class);

            logger.error( "version tracking number = " + VERSION_TRACKING + "." );
            logger.error ("InvocationTargetException exception making RMI method call.");
            logger.error ("itemObject " + (itemObject == null ? "is" : "is not") + " null.");
            logger.error ("name " + (name == null ? "is" : "is not") + " null.");
            logger.error ("args " + (args == null ? "is" : "is not") + " null.");
            if ( connectString != null )
                logger.error ("connectString = " + connectString + ".");
            else
                logger.error ("connectString is null.");
            if ( name != null )
            ...
            logger.error ("Other exception making RMI method call.");
            logger.error ("itemObject " + (itemObject == null ? "is" : "is not") + " null.");
            logger.error ("name " + (name == null ? "is" : "is not") + " null.");
            logger.error ("args " + (args == null ? "is" : "is not") + " null.");
            if ( connectString != null )
