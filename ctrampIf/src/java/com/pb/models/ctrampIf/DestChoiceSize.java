...
public class DestChoiceSize implements Serializable {
    ...
    public void setupDestChoiceSize( HashMap<String, String> propertyMap, String projectDirectory, String tourCategory ){
        ...
        if(propertyMap.containsKey( PROPERTIES_DC_SIZE_OUTPUT )) { 
    ...
    public void calculateDcSize() {
        ...
        if(ModelStructure.MANDATORY_CATEGORY == tourCategory & dcSizeTermOutputFileName!=null) {
        ...
        try {
        	...
        	for( int i=1; i <= numZones; i++ ) {
        		...
        		for( int j=0; j < numSubZones; j++ ) {
        			...
        			// print 0 for empty purposes
                    for( int k=0; k < start; k++ )
                        dataRecord += String.format(",%.5f",0.0);
                    ...
                    // print 0 for empty purposes
                    for( int k=end; k < dcSizePurposeList.length; k++ )
                        dataRecord += String.format(",%.5f",0.0);
